...
/**
	...
	public function list_notice_m($uid = NULL, $top = NULL, $offset = 1, $limit = 30, $ntype =NULL) {
		...
		if ($top) {
			$vals = $this->mongo_db->where_and(array('uid' => $uid, 'is_delete' => 1))->where_lt('dateline',time())
					->select('_id','ntype','stype', 'uid', 'type', 'content', 'dateline')->limit(5)->order_by(array('dateline' => -1))->where_gte( 'dateline' , strtotime('-1 month') )->get('notice');
		} else {
			if(!$ntype)
				$vals = $this->mongo_db->where_and(array('uid' => $uid, 'is_delete' => 1))->where_gte( 'dateline' , strtotime('-1 month') )->where_lt('dateline',time())
		...
		$webinfo = call_soap('interest', 'Index', 'get_web_info',array($webstr));
		...
		foreach ( $vals as $noticev ) {
			...
			if($noticev['ntype'] > 1 && $noticev['stype'] != 'dk_guanzhu_web'){
				...
				$avatar=get_webavatar($contentarray[0],'s',$noticev['ntype']);	//获取头像
				else 
				$avatar=get_webavatar($noticev['uid'],'s',$noticev['ntype']);	//获取头像
			}else {
			...
			if(isset($noticev['stype']) && ( $noticev['stype'] == 'event_c_web' || $noticev['stype'] == 'event_update_web' ||$noticev['stype'] == 'event_ban_web')){
				...
				if(isset($aid) && isset($name1)){
					...
					$userurl='<span class="blueName"><a href="'.mk_url('main/index/index', array('web_id' => $aid),FALSE).'">'.$name1.'</a></span>';
				}else{
					
					 $name = $contentarray[2]['name1'];
					$userurl='<span class="blueName"><a href="'.mk_url('main/index/index', array('action_dkcode' => $dkcode)).'">'.$name.'</a></span>';
				}
			}else{
				
				$userurl='<span class="blueName"><a href="'.mk_url('main/index/index', array('action_dkcode' => $dkcode)).'">'.$uname.'</a></span>';
			}
		...
		if(!$temp ) {
			 $temp['name']="";
			$temp['url'] = mk_url('main/index/index', array('action_dkcode' => $dkcode));
		}else{
			//$temp['url'] = base_url() . substr($temp['url'], (strpos($temp['url'], 'com/')+4));
			if(isset($temp['name']))
				$temp['name'] = shtmlspecialchars($temp['name']);
			if(isset($temp['url1']))
				$temp['url']=mk_url('main/index/index',array('web_id'=>$noticev['ntype']),FALSE);
			if(! isset($temp['url']))
				$temp['url'] = mk_url('main/index/index', array('action_dkcode' => $dkcode));
		}
		...
		if(isset($noticev['stype']) && $noticev['stype'] == 'dk_del_web')  $userurl = null;
			...
			if ($noticev ['type'] == 'photo' || $noticev ['type'] == 'video') {
				$noticev ['type'] = "uiIcon noticeIconPng bp_photo fl";
			} else if ($noticev ['type'] == 'blog' || $noticev ['type'] == 'link') {
				$noticev ['type'] = "uiIcon jewelMiniIcons bpIcon_msg fl";
			} else if ($noticev ['type'] == 'helper') {
				$noticev ['type'] = "uiIcon noticeIconPng bp_ask fl";
			} else {
				$noticev ['type'] = "uiIcon jewelMiniIcons bpIcon_plusMan fl";
			}
	 ...
	 * 截取通知函数
	 */
	function sub_noticestr($uname ,$tem,$temp =  NULL,$tem1 =  NULL,$tmp=NULL,$tmp3=NULL, $tmp4=NULL){
		$mycontent = array();
		$len = mb_strlen($uname);
		$len1 = mb_strlen($tem);
...
//		foreach ($data as $d){
//			$this->mongo_db->insert('notice_type_1',$d);
//		}
//		foreach ($data1 as $d){
//			$this->mongo_db->insert('notice_type_2',$d);
//		}
	}
