...
class FdfsModel {
	...
	function upload_filename($filename = null, $file_ext = null, $meta = array(), $group = null)
	{
		...
		$res = $this->fdfs->storage_upload_by_filename($filename, $file_ext, $meta, $group, $this->tracker, $this->storage);
		if($res){
	...
	function upload_filebuff($filebuff = null, $file_ext = null, $meta = array(), $group = null)
	{
		...
		$res = $this->fdfs->storage_upload_by_filebuff($filebuff, $file_ext, $meta, $group, $this->tracker, $this->storage);
		if($res){
			return $res;
		}else{
			// $error = $this->fdfs->get_last_error_info();
	...
	function upload_slave_filename($filename, $group, $masterfile, $prefixname, $file_ext = null, $meta = array())
	{
		...
		$res = $this->fdfs->storage_upload_slave_by_filename($filename, $group, $masterfile, $prefixname, $file_ext, $meta, $this->tracker, $this->storage);
		if($res){
			return $res;
		}else{
			// $error = $this->fdfs->get_last_error_info();
