...
class Invitecode extends MY_Controller {
	...
	function index(){ 
		...
		$user_info['url'] = mk_url(APP_URL.'/index/index', array('action_dkcode' => $this->dkcode));
		...
		if($restult['recmded_info']){
			...
			$recmded_info[0]['url'] = mk_url(APP_URL.'/index/index', array('action_dkcode' => $recmded_info[0]['dkcode']));
		...
		$this->assign('invitecode_url', mk_url(APP_URL.'/invitecode/index'));
		  ...
		  $html .= '<li class="item" uid="'.$v['uid'].'">';
			...
			$html .= '<span class="userinfo">';
			   $html .= '<p class="name"><a href="'.$v['url'].'" title="'.$v['username'].'">'.$v['username'].'</a></p>';
				$html .= '<div class="statusBox newItem" rel="'.$v['is_follow'].'" uid="'.$v['uid'].'"></div>';
			  $html .= '</span>';
		 $html .= '</li>';
		}
