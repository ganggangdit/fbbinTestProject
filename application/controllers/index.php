    ...
    function index(){
       ...
       /* //获取封面(lvxinxin add)
		...
		if(false == $this->_self){
			...
			$isAllowBase = $this->singleaccessmodel->isAllow('base', $this->action_uid, $this->uid, $this->action_uid);
			$isAllowPrivate = $this->singleaccessmodel->isAllow('private', $this->action_uid, $this->uid, $this->action_uid);
        ...
        $this->assign('msgurl', mk_url(APP_URL.'/msg/index'));
        $this->assign('is_self', $this->_self);
        //加关系部分结束 lanyg 2012/03/21
        
        //获取应用区列表
        $user_app_list = call_soap('ucenter', 'UserMenuPurview', 'getUserPurviewMenu', array('userid' => $this->action_uid, 'nowuserid' => $this->uid));
        ...
        if (is_array($user_app_list)) {
            ...
            foreach ($user_app_list AS $key => $val) {
                ...
                if (isset($menu_param[0]) && $menu_param[0] == 'img') {
                	...
                	$user_app_list[$key]['menu_ico'] = 'http://' . config_item("fastdfs_host") . '/' . $val['group']. '/' . $val['menu_ico'];
                }
