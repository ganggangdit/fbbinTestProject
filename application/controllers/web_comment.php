...
class Web_comment extends MY_Controller
{
    ...
    private function init_web()
    {
        ...
        if($this->web_id) {
			...
			$this->web_info = (isset($tmp) && $tmp && count($tmp)>0) ? $tmp[0] : null;    
    ...
    public function add_comment()
    {
    	...
    	$tid		=$this->input->post('tid');				//首页热度新加
    	$uid		=$this->uid;
    	...
    	if( $object_id && ($content || $content==='0') && in_array($pageType,$this->object_type) ){
        	...
        	$response=call_soap('comlike','Index','add_comment',array($data));
    	 	if($tid){
	        	 call_soap('timeline','Timeline','updateTopicHot',array($fid,1));
	       	}
        	//对相册评论字段操作
			...
			switch ($pageType){
				...
				case 'web_topic':
					...
					if(!$msgurl) $msgurl=mk_url($this->msgurl[$pageType],array('tid'=>$tid,'web_id'=>$this->web_id,'from'=>'web'),false);
					break;
				case 'web_video':
