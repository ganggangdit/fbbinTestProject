...
class MY_Log extends CI_Log {
	...
	public function __construct() {
		...
		if(!self::$db){
			self::$db = new Mongo_log($this->config);
		}
		$this->mon = self::$db;
		if($this->config['mongo_log_dbname']){
			$this->mon->switch_db($this->config['mongo_log_dbname']);
		}else{
			$this->mon->switch_db($this->dbname);
		}
	}

	public function write_log($level = 'error', $message='',$filepath='',$line='', $php_error = FALSE) {
		$config =$this->config;
		...
		if(in_array($level_error,array('ERROR','DEBUG','INFO','ALL'))){
			$collection = $this->select_collection($level_error);
			$doc = array(
				'type'=>$level,
				'filepath'=>$filepath,
				'msg'=>$message,
				'line' => $line,
				'time'=>date($this->_date_fmt),
			);
			$this->mon->insert($collection,$doc);
		}
	}
	 ...
	 */
	public function write_user_log( $uid = '',$msg = array(), $time=''){
		if(!$time){
...
class Mongo_log 
{
  ...
  public function delete_all($collection = "")
  {
	...
	{
		//$config = &get_config();
		$this->host = trim($config['mongo_log_host']);
		$this->port = trim($config['mongo_log_port']);
		$this->user = trim($config['mongo_log_user']);
		$this->pass = trim($config['mongo_log_pass']);
		$this->dbname = trim($config['mongo_log_dbname']);
		$this->persist = trim($config['mongo_log_persist']);
		$this->persist_key = trim($config['mongo_log_persist_key']);
