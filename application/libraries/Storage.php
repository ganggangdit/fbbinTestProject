...
class Storage
{
	...
	{
		$file_info = $this->fdfs->storage_upload_by_filename($local_filename,$file_ext,$meta,$this->group,$this->tracker,$this->storage);
	...
	{
		$file_info = $this->fdfs->storage_upload_by_filebuff($file_buff,$file_ext,$meta,$this->group,$this->tracker,$this->storage);
		if(is_array($file_info))
		{
			return $file_info;
		}
	...
	public function uploadSlaveFile($local_filename,$master_filename,$prefix,$file_ext = null, $meta = array())
	{
		...
		$res = $this->fdfs->storage_upload_slave_by_filename($local_filename, $this->group, $master_filename, $prefix, $file_ext, $meta, $this->tracker, $this->storage);
		if($res){
			return $res;
		}else{
	...
	{
		$file_info = $this->fdfs->storage_upload_slave_by_filebuff($file_buff,$this->group,$master_filename,$prefix,$file_ext,$meta,$this->tracker,$this->storage);
		if(is_array($file_info))
		{
			return $file_info;
		}
	...
	{
		return $this->fdfs->storage_download_file_to_file($this->group,$remote_filename,$local_filename,0,0,$this->tracker,$this->storage);
	}
	...
	{
		$buff = $this->fdfs->storage_download_file_to_buff($this->group,$remote_filename,0,0,$this->tracker,$this->storage);
		if($buff !== false)
		{
			return $buff;
		}
	...
	function file_exist($group = null, $name = null)
	{
		if(!$group or !$name)
			return false;
		$res = $this->fdfs->storage_file_exist($group, $name, $this->tracker, $this->storage);
		if($res) {
			return $res;
		}
		return false;
	...
	public function deleteFile($group, $filename)
	{
		...
		$res = $this->fdfs->storage_delete_file($group, $filename, $this->tracker, $this->storage);
		if($res) {
			return $res;
		} else {
			// $error = $this->fdfs->get_last_error_info();
